...
if command -v yay >/dev/null 2>&1; then
    echo -e "\e[32myay is already installed, skipping installation.\e[0m"
else
    echo -e "\e[34m🔧 Cloning yay...\e[0m"
    cd "$HOME" >>"$HOME/HyprArch/log.txt" 2>&1 || {
        echo -e "\e[31mFailed to change directory to $HOME\e[0m"
        exit 1
    }

    git clone https://aur.archlinux.org/yay.git "$HOME/yay" >>"$HOME/HyprArch/log.txt" 2>&1 || {
        echo -e "\e[31mFailed to clone yay\e[0m"
        exit 1
    }

    cd "$HOME/yay" >>"$HOME/HyprArch/log.txt" 2>&1 || {
        echo -e "\e[31mFailed to change directory to $HOME/yay\e[0m"
        exit 1
    }

    echo -e "\e[34mBuilding and installing yay...\e[0m"
    ...
    makepkg -si --noconfirm >>"$HOME/HyprArch/log.txt" 2>&1 || {
        echo -e "\e[31mFailed to install yay\e[0m"
    ...
    echo -e "\e[32myay installed successfully.\e[0m"
    exit 0
fi
exit 0
