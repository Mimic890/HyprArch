...
chsh -s /usr/bin/zsh || {
    echo -e "\e[31mFailed to set zsh as the default shell.\e[0m"
    exit 1
}
# Install zinit
...
if [ -f "$HOME/HyprArch/customs/zsh/starship.toml" ]; then
    ...
    echo -e "\e[31mstarship.toml not found.\e[0m"
    exit 1
fi
# Copy .zshrc config
