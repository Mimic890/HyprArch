...
# --- LOGGING FUNCTIONS ---
# Functions for color-coded console output.
...
# Path to the folder with your custom themes
THEME_SOURCE_DIR="$HOME/HyprArch/customs"

# Path to the configuration files directory.
CONFIG_DIR="$HOME/.config"

# List of available themes (filenames without the extension)
declare -a themes=("blue_arch" "black_arch" "white_arch" "amoled_arch" "purple_arch" "orange_arch")

# --- FUNCTIONS ---

# Function that demonstrates all available colors
show_all_colors() {
    log "--- Standard Colors ---"
...
show_theme_menu() {
    ...
    echo -e "${WHITE}------------------------------------------${RESET}"
    # Dynamically create the menu from the themes array
    for i in "${!themes[@]}"; do
...
# Function to apply a theme to a specific application
apply_theme() {
    local app_name=$1
    local dest_dir=$2
    local style_file=$3
...
# (Optional) Uncomment the next line to see all colors when the script starts
# show_all_colors

# Show the menu and prompt the user for a choice
show_theme_menu
...
# Get the name of the selected theme from the array
selected_theme=${themes[$((choice-1))]}
printf "\n${B_GREEN}Selected theme: %s. Applying...${RESET}\n" "$selected_theme"

# Apply themes for each application
...
apply_theme "nwg-dock-hyprland" "$CONFIG_DIR/nwg-dock" "style.css" "css" "$selected_theme"

ok "\nAll themes have been applied successfully!\n"
